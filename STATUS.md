...
| 项 | 状态 |
|---|---|
| 本地 Git 基线 | **73ee74c（新公开仓库 #34）**；CRM 持久化已合入，旧历史及原候选保留。 |
| 本轮交付 | 可信快照、保存分析与驾驶舱引用已交付；完整后端 2810 passed/77 skipped/71 deselected、B0、合成浏览器 QA 及 PR CI 通过。运行与真实账号验收见[交付记录](docs/crm-calibration/crm-assets-release-2026-09-21.md)。 |
...
- 已确认的短日期窗口只读核对通过：GSV 与先前基准一致，订单/买家与独立聚合一致。归档大小和修改时间未变；真实账号 MiniMax 查询和可信快照一致，界面确认保存/引用、刷新及全局驾驶舱重开通过。
- #34 的 PR 与 main CI 均通过，受控切换和旧版实际回退已完成；保留原 runtime、历史和 DuckDB 1.5.3，归档大小/mtime未变。当前三步完成，局部真实验收不代替本人 UAT。
...
- 多格式编辑的完整固定 DSH 宿主浏览器证据使用合成会话和文件；该功能的真实历史/模型/业务及本人视觉/UAT 仍待验收；CRM 局部链以上述本轮记录为准。扫描PDF的OCR、复杂Office兼容性及PDF浮动菜单异常仍开放。
