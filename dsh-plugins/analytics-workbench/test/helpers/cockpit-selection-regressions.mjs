...
export async function checkSelectionRegressions({ page, idle, click, check, shot }) {
  ...
  await click('放弃本次修改'); await save('Edited heading');
  ...
  assert.equal(await frame.locator('#dynamic').getAttribute('data-cockpit-target'), null);
  ...
  await page.waitForFunction(() => document.querySelector('[data-testid="html-preview-patch"]')?.disabled === true);
  ...
  await click('放弃文本修改');
  assert.equal(await page.getByRole('button', { name: '用 AI 修改此选区', exact: true }).isDisabled(), true);
  assert.equal(await page.evaluate(() => window.cockpitFixture.pageStore.getSnapshot().current.version), 1);
  ...
  check('R3：脚本改写区域从画布与下拉同步移除，延迟改写禁用文本/AI 提交并保留草稿');
  ...
  await click('完成编辑'); await click('用 AI 改'); await click('放弃本次修改');
  ...
  await page.unroute(endpoint); await click('用 AI 改');
