...
const delivery = createCockpitDelivery({
  ...
  readBytes: async (sessionId: string, path: string, options: { offset: number; length: number }) => {
    const text = (files[sessionId as keyof typeof files] as Record<string, string> | undefined)?.[path];
    if (text === undefined) throw new Error('fixture resource missing');
    const bytes = new TextEncoder().encode(text);
    const chunk = bytes.slice(options.offset, options.offset + options.length);
    return { ok: true, value: { path, offset: options.offset, bytes: bytes.length, version: 'fixture-v1', eof: controls.eof,
