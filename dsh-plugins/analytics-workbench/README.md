...
- 驾驶舱通过认证的 `POST /api/shine-mage-deliveries` 冷读原生历史会话，按来源搜索、分页、物理路径去重，并显示部分失败；不恢复旧会话的Agent运行。
- 「添加产物」与「保存副本并编辑」支持HTML、Word、Excel/CSV、PDF，使用 `/api/v1/analytics/cockpit-files` 私有文件库；原件和正式版本保留。手动HTML使用 `origin_file_id`，没有来源会话时 `session_id=null` 且不可绑定业务结果。
- Office/PDF通过固定9.4.0本机Docker ONLYOFFICE编辑；配置 `COCKPIT_OFFICE_CONFIG`、`COCKPIT_OFFICE_ENV_FILE`，Compose端口可由 `COCKPIT_OFFICE_PORT` 指定，默认仅 `127.0.0.1:18110`。配置、上传限制、保存回执/冲突恢复及PDF入口见 [本地文档服务](../../docs/operating/cockpit-office-local.md)。
- 「用AI改」复用唯一DSH原生Loop，`/api/v1/analytics/cockpit-ai` 创建指定版本副本任务；收取后候选固定，只读对比再显式保存。绑定HTML仅允许CSS修改。步骤、任务分页/状态、API与过期恢复见 [AI修改说明](../../docs/operating/cockpit-ai-edit.md)。
- 本轮合成后端2678 passed/77 skipped/71 deselected；B0另567项Python及所选Node/DOM/契约/类型/构建/干净重建通过，两层不相加。前轮真实浏览器使用合成会话/文件与候选，真实模型返回 `MISSING_CREDENTIAL`；P13、真实历史/业务、UAT及现役切换未验收。产品仍PARTIAL。

## 本轮组件库合同与实现（2026-09-14）

目标与验收以 [TODOS 的 M1 核心交付](../../docs/hackathon/TODOS.md#m1-核心交付) 为准，代码落点与CI见 [STATUS](../../STATUS.md)及 [#134](https://github.com/tyuanww/fuqing-crm-analytics/pull/134)。9 月 15 日 S3 原型与正式壳续验见 [S3-ACCEPTANCE](../../docs/hackathon/S3-ACCEPTANCE-2026-09-15.md)。下方分项测试数／“待验”是各阶段历史证据语境，当前停点以 STATUS/TODOS 为准；不代表无代码平台已经完成。

目录 SSOT：[`src/board-spec/component-catalog.json`](src/board-spec/component-catalog.json)，版本 `board-components/v1`。新组件块显式携带 `library_version`，由 `parseBoardSpec` 校验、`interpretBoard` 调用只读投影、`LibraryComponentBody` 渲染；运行时无 Figma 请求。原生 Connection 环境已接新版画布及目录/生成工具；无该服务的旧 fixture 仍走兼容渲染。S2-C1 已存板 catalog 已合 [#136](https://github.com/tyuanww/fuqing-crm-analytics/pull/136)；2026-09-14 同一 6677 上其余五类编辑、LINE 换数与当前模型板回退已记账。开放项（正式壳 B3 九态仍 PARTIAL、G1 本候选真模型、UAT、G1–G6）看 STATUS/TODOS。[S4](../../docs/hackathon/S4-QA-2026-09-15.md)／[S5](../../docs/hackathon/S5-UAT-HANDOVER-2026-09-15.md) 已钉 6677 候选；现役入口不是 4325。R-1 预检已合 [#153](https://github.com/tyuanww/fuqing-crm-analytics/pull/153)。隔离 HTTP 集成在已加载构建上命中 TABLE `show_values`→`COMPONENT_PROPERTY`；现场未再造该缺陷。不要求重新选模型。2026-09-19 V2 候选有 library 时，侧栏驾驶舱 `main` 由 `LibraryCockpitPanel` 转交 `CockpitWorkspace`：左栏合并已存自由页、看板与捕获来源会话的工作区 html/xlsx/csv/pdf，文件列表显示刷新状态与截断信息。工作区 HTML 可预览并显式保存副本；已存 HTML 仅在可靠源码映射处支持点选文字和 `replacementText` 精确替换，仍使用 FREE_PAGE CSP、no-referrer 与原沙箱。页头「编辑」打开 `CockpitSidebar`；看板点选调用 `selectComponent`，字段预览调用 `previewBlockPatch`，只有显式「用 AI 改」才进入原生会话。无 library 时仍走 Legacy 样例板。使用步骤、脚本/映射限制和隔离验证范围见 [V2 交付说明](../../docs/hackathon/COCKPIT-V2-LOCAL-2026-09-19.md)；不改 DSH 上游。
...
`cockpit-composition.mjs/tsx` 使用官方 `shell.overlay` 放置画布；原生 `main/conversation` 继续拥有唯一会话、消息和 composer，原生 rightbar 检查器保留。集中适配只识别钉住壳的 `main → main.conversation` 两层 `display:contents` 插槽包装，给原生根设置本插件拥有的属性/CSS 变量，不克隆、搬移或重建聊天。结构不符明确提示独立驾驶舱备选，不强行覆盖未知壳。
