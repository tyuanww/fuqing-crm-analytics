...
`cockpit-composition.mjs/tsx` 使用官方 `shell.overlay` 放置画布；原生 `main/conversation` 继续拥有唯一会话、消息和 composer，原生 rightbar 检查器保留。集中适配只识别固定 `ddefc45f` 的 `main → main.conversation` 两层 `display:contents` 插槽包装，给原生根设置本插件拥有的属性/CSS 变量，不克隆、搬移或重建聊天。结构不符明确提示独立驾驶舱备选，不强行覆盖未知壳。
