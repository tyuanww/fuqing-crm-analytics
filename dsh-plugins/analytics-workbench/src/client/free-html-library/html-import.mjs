...
export async function convertWorkspaceHtml({
  html, path, sessionId, artifactId, readResource,
  maxResources = DEFAULT_IMPORT_RESOURCE_MAX,
  maxResourceBytes = DEFAULT_IMPORT_RESOURCE_BYTES,
} = {}) {
  if (!sessionId && !artifactId) return fail('NO_SESSION', '导入需要明确来源会话或上传文件');
  ...
  return {
    ...
    origin: { session_id: sessionId ?? null, path, ...(artifactId ? { file_id: artifactId } : {}) },
