...
const SHEET_EXT = /\.(xlsx|xls|ods|csv)$/i;
const PDF_EXT = /\.pdf$/i;
const DOC_EXT = /\.(docx|doc|odt|rtf)$/i;
...
export function cockpitProductIdentity(item) {
  ...
  if (typeof item.file_id === 'string' && item.file_id) return `cabinet:${item.file_id}`;
...
export function mergeCockpitProducts({ files = [], boards = [], pages = [] } = {}) {
  ...
  for (const file of files) {
    ...
    if (!identity.startsWith('file:') && !identity.startsWith('cabinet:')) continue;
    push({
      ...file,
      id: identity,
