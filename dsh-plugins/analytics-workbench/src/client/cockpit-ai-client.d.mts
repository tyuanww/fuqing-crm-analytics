...
export type AIJob = { id: string; target_kind: 'file' | 'page'; target_id: string; base_version: number; filename: string;
...
export type AIState = { jobs: AIJob[]; active: AIJob | null; busy: boolean; confirmationUncertain: boolean; message: string;
...
export function createCockpitAIClient(http?: PageHttpOptions | null, adapters?: {
  ...
  refresh(): Promise<void>; select(kind: string, id: string): void; begin(kind: string, id: string, version: number): Promise<boolean>;
