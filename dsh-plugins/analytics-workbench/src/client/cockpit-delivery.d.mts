...
export type DeliverySnapshot = {
  ...
  lists?: number;
  history?: { examined?: number; totalSessions?: number; nextCursor?: string | null; failures?: unknown[]; unsupportedPaths?: number; error?: string };
};
...
export function createCockpitDelivery(adapters?: {
  ...
  max?: number;
  dirDepth?: number;
  history?: (cursor: string | null, signal?: AbortSignal) => Promise<{ files: CockpitProduct[]; examined: number; totalSessions: number; nextCursor: string | null; failures: unknown[]; unsupportedPaths: number }>;
  artifactInbox?: { intake(input: Record<string, unknown>): Promise<unknown> };
}): {
