...
> **候选追加**：4325/18083 均为 `84b0ac8`，模型配置和原资产保持，现有 2 块板、10 条结果、原草稿 v2；单位/标识/自动保存表述在本次真实用例通过，完整 T13 及 B0 404 仍开放，见[当前记录](./docs/hackathon/COMPUTED-UNITS-CANDIDATE-2026-09-10.md)。
...
独立 CRM 插件提供原生对话登录、看板 GSV、正额购买口径的 AOV/AUS，以及服务端可信快照、账号私有保存分析、检索/分享/组板和驾驶舱固定引用。本分支候选另含看板就绪度、成交时会员溢价、净额 GSV 与教材文档 grants，未加载 6677。销售/回购/派样的 `crm-metrics/v1` 工具仍只接受合成源。使用与复验见 [CRM 插件说明](./dsh-plugins/crm-knowledge/README.md)，0.14 候选见[整合记录](./docs/crm-calibration/crm-integration-2026-09-21.md)；已合入的快照/保存见[持久化记录](./docs/crm-calibration/crm-assets-release-2026-09-21.md)；旧交付范围见[历史说明](./docs/crm-calibration/public-release.md)。真实 GSV、文档检索和教材图谱已有局部模型证据；完整真实源映射、Word/HTML 多人权限、全图语义与本人 UAT 仍开放。

新 DSH 开发入口见 [dsh-dev 使用说明](./scripts/dsh-dev/README.md)（现役代码与 runtime 分属不同仓库，先核对该说明中的当前归属；不要 `--fresh` 或用原仓旧源码默认 `reload`）。侧栏驾驶舱是独立二级页产物柜，V2 已随 #214 合入。已合 #215 的 0.11.0.0 交付汇总历史会话产物，支持手动添加 HTML、Word、XLSX/CSV、PDF；Office/PDF 使用本机 Docker ONLYOFFICE 编辑，原件与版本独立保存。操作与配置见 [本地文档服务](./docs/operating/cockpit-office-local.md)。点击「用 AI 改」进入唯一 DSH 原生 Loop 的版本副本任务，再收取候选、只读预览并显式确认，见 [AI 修改步骤](./docs/operating/cockpit-ai-edit.md)。0.13.0.0 已交付回收站、整个产物面板移动/缩放/停靠、全屏展示与静态 HTML 选区编辑；原生 MiniMax-M3 五项合成选区评估通过；reload 后真实 DSH 驾驶舱及新入口可见性检查通过，完整编辑旅程、逐格式真实模型、P13、真实历史/业务及用户 UAT 仍开放。旧轮 `MISSING_CREDENTIAL` 失败保留在[历史 Ship 记录](./docs/hackathon/COCKPIT-SHIP-2026-09-20.md)，本轮证据见[UI / UX 报告](./docs/hackathon/COCKPIT-UI-UX-2026-09-20.md)。BoardSpec 生成见 [插件 README](./dsh-plugins/analytics-workbench/README.md)。现役插槽见 [DSH-PLUGIN-UI-COMPAT](./docs/hackathon/DSH-PLUGIN-UI-COMPAT.md)。登录菜单里的「比赛看板」在新标签页打开 `https://board.tyuan.chat/`，看板仍独立，DSH 不内嵌也不代理。`start-stack` 的本地前端仍是 `http://127.0.0.1:15173`。比赛 HTTP 见 [competition-http](./docs/operating/competition-http.md)，验证按 [当前验证入口](./docs/operating/verification.md) 选择范围。以下是**既有演示/私有分析的历史入口**，不作为新工作台的默认启动或验证命令；不要继承私人数据库配置。
...
- 前端界面: http://127.0.0.1:15173（`start-stack` 默认；裸 `npm run dev` 仍可能是 5173）
- API 文档（无需登录）: http://127.0.0.1:8000/docs
