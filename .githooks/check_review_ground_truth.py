...
# Exact inherited documents from the reviewed public bootstrap snapshot. These
# are historical records, not fresh implementation claims. Any indexed edit or
# manifest change restores ordinary checking (also in committed/CI mode).
PUBLIC_SNAPSHOT_MANIFEST = ".githooks/public-snapshot-docs.sha256"
PUBLIC_SNAPSHOT_MANIFEST_SHA = "4dde9763d2a4d0a0d3a3917dc25d0150ff7afc203f456fd0665cb8cba32fa194"


def verified_public_snapshot_document(path: str, committed: bool = False) -> bool:
    if not is_review_file(path):
        return False
    revision = "HEAD:" if committed else ":"

    def read_git(name: str) -> bytes:
        result = subprocess.run(
            ["git", "show", revision + name], capture_output=True, check=False,
        )
        return result.stdout if result.returncode == 0 else b""

    manifest = read_git(PUBLIC_SNAPSHOT_MANIFEST)
    if hashlib.sha256(manifest).hexdigest() != PUBLIC_SNAPSHOT_MANIFEST_SHA:
        return False
    for line in manifest.decode("utf-8").splitlines():
        digest, name = line.split("  ", 1)
        if name == path:
            content = read_git(path)
            return bool(content) and hashlib.sha256(content).hexdigest() == digest
    return False
...
def check_file(path: str, committed: bool = False) -> list[tuple[int, str, str]]:
    ...
    if verified_frozen_plan(path, committed) or verified_public_snapshot_document(path, committed):
        return violations
    if committed:
