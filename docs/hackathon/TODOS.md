...
### 2026-09-20 CRM 指标、知识与原生对话登录（v0.12.0.0 候选）
...
- [ ] 当前来源和 `query_ref` 尚未接入保存分析/驾驶舱持久化引用；真实模型、完整D019映射、现役挂载、旧服务迁移、私有ACL和WeKnora/Neo4j仍待验收。
- [ ] 文档同步后由主任务创建PR并核验远端CI；代码合并不等于上线或真实业务闭环，产品仍PARTIAL。
