...
PR #20 首次 Linux CI 的磁盘溢写测试另触发了 RSS 保护：临时文件约 53.7 MiB，低于 64 MiB 配额，但进程 RSS 259 MiB 超出测试自设的 256 MiB。仅将此夹具的进程余量调至 512 MiB，保留 32 MiB 引擎预算和 1/64 MiB 磁盘配额，并断言失败分支不能由 RSS 超限冒充磁盘配额生效。独立 32 MiB RSS 终止测试及运行时 1024 MiB 上限不变。

## 全部 PR 队列续批 #21–#28

用户要求继续处理所有 PR。本候选接收 #21 Rive 2.42.2、#22 PostCSS 8.5.28、#23 chardet 7.6.0、#24 Vue Test Utils 2.5.1、#25 wheel 0.48.0、#27 pytest-xdist 声明下限 3.8.0、#28 typing-inspection 0.4.4。pytest-xdist 是开发声明下限，本次不把它加入生产锁，也不将声明变更冒充已启用并行执行。

#21 原 CI 失败是两份托管 WASM 仍来自旧 Rive 版本。现在从锁定的 npm 包同步 `public/riv/rive.wasm` 和 `rive_fallback.wasm`，保留原字节一致性测试及同源 CSP；没有放宽断言或改用外部 CDN。升级命令为 `npm ci` 后从 `node_modules/@rive-app/canvas/` 同步这两个文件。

#26 的 core 2.48.0 仍与 Pydantic 2.13.5 精确要求的 2.46.5 冲突，不能合并。此前只忽略 2.49.0，导致机器人回退提出同样不兼容的 2.48.0；现改为有界忽略 `>2.46.5,<2.50.0`，恢复条件仍是父包升级后同步核对其精确 core 版本。该项按不兼容关闭，不标记升级成功。

本轮审查、组合验证与合并状态以关联 PR 为准；现役服务、归档库和 B0 的独立固定锁不随此次升级切换。

## 自动队列补充 #30–#32

#29 合并后生成的 #30 Ruff 0.16.8、#31 Vue Query / Query Core 5.103.1、#32 Axios 1.20.0 继续纳入处理。声明与锁同步，Query Core 保持与 Vue Query 精确配套，Ruff 规则选择仍保留 `E4/E7/E9/F`。本轮不改 API 拦截器、权限、查询筛选和缓存代码；运行相应组合测试，结果以关联 PR 为准。历史供应链文档中的日期快照不是当前依赖锁，精确版本以本次清单/锁文件为准。

#33 首次 CI 的取消测试在 POST cancel 收到明确可重试的 `503 STATE_UNAVAILABLE`，但夹具只处理了版本冲突 409。测试现在仅对该明确瞬态错误最多发送三次相同请求；409 仍单独重读版本，其他错误或持续 503 仍失败。新增取消阶段真实 SQLite 排他锁，与启动占锁和过期版本组合覆盖，断言瞬态重试保持幂等键和 CAS 版本、只有一个 worker，最后物理执行退出且活动槽释放。没有修改运行时取消逻辑或降低成功条件。
