...
模型查询产生服务端可信快照；用户在“CRM 分析”核对后确认保存分析，再确认加入驾驶舱。浏览器写入仅带来源 ID 和标题，不能提交金额或账号。固定结果按 CRM 账号隔离，重开及新进程读取保留原结果；查询重做产生新快照。配置、接口、重试与容量说明见[插件使用说明](../../dsh-plugins/crm-knowledge/README.md#保存分析检索分享与组板)。
