...
仍开放：全图语义审核、多用户文档ACL、其他真实指标、图与原文同版本证明。销售快照、保存分析/驾驶舱固定引用已由新公开仓库 #34 实现，其运行与真实账号验收见[本轮记录](crm-assets-release-2026-09-21.md)；本次不把教材图谱关系保存为可信销售事实。真实业务口径与接入缺口见[真实指标接入清单](metric-readiness.md)。
