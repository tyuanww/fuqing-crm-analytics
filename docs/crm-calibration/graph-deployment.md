...
公开主线为 `64eb6e06`（v0.13.0.0）。本轮在已合图谱查询上补质量账本、原文版本绑定和 CRM 账号文档权限。不修改 DSH 上游，不读 CRM 归档库，不扩大到未授权知识库。Neo4j 自动抽图、受限查询工具、真实模型调用、全图语义审核是分开验收；局部通过不能宣称全图完成。
...
仍开放：其余 1113 条关系的人工核对、抽取时文本哈希、宿主中共享 weknora_search 的卸载、CRM 分析共享实现、其他真实指标。销售快照/保存分析见[本轮记录](crm-assets-release-2026-09-21.md)；不把教材图谱关系保存为可信销售事实。真实业务口径与接入缺口见[真实指标接入清单](metric-readiness.md)。产品仍 PARTIAL。
