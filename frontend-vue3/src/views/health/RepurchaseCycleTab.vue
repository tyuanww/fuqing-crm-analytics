  ...
  return {
    tooltip: {
      ...
      formatter: (p: HeatmapParam) => {
        const cohort = encodeHtml(yData[p.data[1]] ?? '')
        const period = encodeHtml(xData[p.data[0]] ?? '')
      ...
      <NGrid :cols="5" :x-gap="12" responsive="screen" class="mb-4">
        ...
        <NGi>
          ...
          <div class="bi-card bi-card-hover px-3 py-3 text-center">
            ...
            <p class="text-2xl font-bold text-slate-900">{{ formatPercent(data.all_store_repurchase_rate) }}</p>
