...
1. 在 Windows 主机启用 WSL 2、Docker 和 Windows OpenSSH。Tailscale 先安装在 Windows 主机，管理员从 Mac 通过 Tailscale SSH 进入，再执行 `wsl.exe -d <distro>`。不要把 SSH、6677、8000、18090、18091、18092、18093 直接暴露到公网。
...
10. CRM、DSH、WeKnora 都通过 Tailscale 完成内部验收后，才安装 Cloudflare Tunnel。`cloudflared-config.yml.example` 只暴露 `www`、`app`、`page` 三个入口；知识库和图数据库保持内部访问。
