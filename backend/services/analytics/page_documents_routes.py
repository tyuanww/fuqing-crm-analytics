...
from backend.services.analytics.cockpit_files import CockpitFileStore, MAX_FILE_BYTES
...
class _PageBodyLimit:
    ...
    async def __call__(self, scope, receive, send):
        ...
        while True:
            ...
            limit = MAX_FILE_BYTES if scope.get("path", "").startswith("/api/v1/analytics/cockpit-files") else PACKAGE_MAX_BYTES
            if len(body) > limit:
                response = _error(AnalyticsError(413, "FILE_SIZE" if limit == MAX_FILE_BYTES else "PACKAGE_TOO_LARGE",
                                                "文件不能超过 20 MB。" if limit == MAX_FILE_BYTES else "页面源码包超过大小上限。"), request_id)
...
def create_page_app(
    ...
    *,
    ...
    if page_state_dir is not None:
        ...
        app.include_router(cockpit_ai_router(CockpitAIStore(files, store), principal, office, office_principal))
