...
- 自由 HTML 生成、工具卡片兜底和原生工作区/呈现扫描统一走候选收件箱；驾驶舱显示待处理数量，确认后才创建正式页面。
- 宿主没有实时事件时，驾驶舱使用 2 秒一次、最多 20 秒的会话扫描，并保留手动刷新。
- 未登录账户菜单在「比赛看板」下新增「知识库」入口，打开杭州 WeKnora 的 `https://learn.tyuan.chat/`；WeKnora 继续负责登录与权限。
...
- 仍不修改 DSH 上游源码，也不改变 `free_html_page_generate` 工具合同。

### Fixed

- 新人入口改到维护地图；README 的快速开始不再把旧的 8000 演示栈当成第一步，当前提交号只留在 STATUS。
- 驾驶舱编辑侧栏改为白底黑字；CRM 连接条的按钮和说明改为黑色，避免淡紫色看不清。脚本生成的卡片句子可以改周围的字，行内指标标签仍保留。
...
- 驾驶舱所依赖的 DSH 钉升到 0.1.7-alpha.2。读工作区文件字节时按新接口提交范围，并接受原始字节。现役 6677 在重载前仍用上一钉。
...
- 编辑选区的脚本在压缩后仍能运行，页面上的数字可以点选。同一标签里的第 n 个节点也可以点选。
- 从 `https://app.tyuan.chat` 打开驾驶舱产物时，页面接口会放行这个来源。
- 对话没有接受这次 HTML 生成时，会立刻告诉你，不再空等页面包。

### Scope

- 不改 DSH 上游。保存分析和加入 CRM 引用仍由用户在「CRM 分析」里确认。知识库不提供生意数字。

## [0.16.0.1] - 2026-09-22

### Fixed

- 驾驶舱里带指标标签的整段说明可以一起改。编辑时能看到原来的数字，改周围的字时这些数字仍留在原来的标签里。
- CRM 连接成功后，对话输入区显示分析图标和「CRM分析」，并用浅色高对比样式。
...
- 驾驶舱可以按正式编辑合同预览并确认文字、样式、属性和当前节点的结构替换。样式和属性不改页面源码，结构替换在确认后才写入。
- 打开 `FQ_FREE_PAGE_SIDECAR_INDEX` 后，节点列表、文字预览和补丁定位使用同一张生产 Node Graph。没有 shine 锚点的历史元素可以按 source range 预览。

### Fixed

- 推断节点确认后，画布和替换框会显示新文字，页面 HTML 保持原字节。
- 同名嵌套标签按深度配对；样式和属性值留在引号内。

### Scope

- 开关默认关闭。运行时选区、iframe、canvas 和绑定节点仍只可查看。本地校验不是真实 cockpit、真实模型或 6677 验收。

## [0.15.0.0] - 2026-09-21

### Added

- 脚本生成的卡片文案与混合文字可独立保存；筛选重渲染后按目标身份重新应用，无法匹配时提示，保留原脚本和版本历史。
- HTML 块级 AI 支持页内填写要求、进入原生主对话及右侧产物预览；手动与 AI 共用版本化显示记录，后端拒绝越界修改。详见 [实现与验收边界](docs/hackathon/COCKPIT-PRESENTATION-2026-09-21.md)。

### Fixed

- 选区无法对应页面源码时给出明确错误，避免静默无操作。
- 预览运行时在文字节点离开文档后不再因空目标中断。
- 右侧产物标签关闭重建后仍能核对未确认的候选。

## [0.14.0.0] - 2026-09-21

### Added

- 已保存分析可按标题、说明或编号检索并分页；可改标题和说明，金额、分母、口径和原始快照保持当时结果。
- 分析可分享给明确的 CRM 账号并撤销；撤权后对方现有登录也不能再读取。不生成公开链接。
- 可将 GSV、AOV、AUS 等指标放入独立组板，保存布局和展示属性；金额始终来自已保存快照。
- 看板增加认证后的就绪度、成交时会员溢价和净额 GSV 查询。缺成交时身份或退款事件时返回不可用，不使用当前会员标记或行上退款代替。
- 教材图谱按当前 CRM 账号检查文档授权；含知识引用的分析在保存、分享和读取时重查权限。纯销售指标分析不绑定教材。

### Fixed

- 教材授权被收回后，不能再用新的保存或驾驶舱引用请求把同一条分析读回来。
- 先分享再绑定知识引用时，对方若无权阅读该教材，分享会被清掉。
- 图谱检索在命中缺少知识库编号时不再放行。
- 净额 GSV 就绪检查把退款 `status` 列为必备字段，与成功退款查询一致。

## [0.13.0.0] - 2026-09-21

### Added

- 驾驶舱产物可移入回收站并恢复；整个产物面板可自由移动、缩放和停靠，位置与列表顺序随账号保存。
- 增加全屏展示，精简产物列表与标题、版本及已结束的 AI 提示，让页面预览获得更多空间。
- 普通静态 HTML 可保存为可编辑副本，点选改字或选择上级板块交给 AI 修改，预览后保存新版本。

### Fixed

- 修复 HTML 预览显示字面量 `</header>`、混合映射选择错位、SVG 前后静态文字无法编辑，以及首击选区和首尾空格丢失。
- 动态区域、可执行链接及不可靠结构保持只读；选区候选校验原版本、哈希、根标签与上下文，拒绝越界修改和过深嵌套。
- 修复删除后重新进入出现已移除产物、删除或恢复后焦点丢失、排序键盘焦点中断及新 AI 错误被旧提示遮挡。

## 2026-09-20 — 依赖 PR 联合收尾候选

- 联合校验 VueUse 15、PyArrow 25 及七项配套依赖升级；DuckDB 新锁仅用于隔离环境，真实归档服务升级须另行完成备份恢复验收。
- js-yaml override 更新至 4.3.2，修复开发工具依赖的 GHSA-2883-xcg3-v3hh。
- 保留 TypeScript 6 与完整 vue-tsc 检查；TypeScript 7 因已复现的上游编译器 API 不兼容暂缓，停止重复自动提案。
...
- Preserve normal hooks; verify imported historical document fingerprints and restore normal checking on edits.
- Use the shared ratio formatter for six legacy health-page values; keep raw API/Excel ratios.

- CRM 图谱：新增可选的 Neo4j 教材关系查询，逐次复核 WeKnora 文档权限和引用片段，区分原文核对、待核对与已排除关系；本机 MiniMax 实际调用已验收，全图语义审核仍未完成。
- CRM 知识状态：过期验证指纹不再误报“未实现”，离线快照与当前合成验证分别展示；新增可复现 preset 生成入口，修复长驻宿主读取旧模块的问题。
- CRM 指标口径：真实 AUS/AOV 默认以当前看板 GSV 为分子，扣退款净额版单列。真实分母覆盖与查询接口尚待验收，现有看板金额未变。
- 侧栏登录/主题：收起后主题钮叠在 rail；未登录显示「未登录」，登录菜单可填显示名称，写入后显示名字/飞书。rail 选择器改 `:has(.sm-login[data-wide="0"])`。不是飞书 OAuth。不改 DSH 上游。不上公网。
...
- 新增独立 CRM 插件：在 DSH 原生对话内连接 CRM，按当前账号查询既有看板 GSV；凭据仅在宿主内存中管理，不进入模型工具参数。
- 新增 `crm-metrics/v1` 销售、老客回购、派样跟踪查询，以及受控合成 DuckDB 只读适配、独立缓存、OpenAPI 和调用方类型。
- 新增 CRM 离线知识包与解释工具，区分目标口径、历史代码、未决冲突和合成验证状态；公开包不含私有教材原文或真实业务数据。

### Fixed

- 修正查询种类之间的缓存串用、闰年同比窗口、商品筛选范围、未知买家的人均分母，以及退款无法归属和历史不完整时的错误计算。

### Scope

- 现有看板 GSV 沿用已确认口径，与扣退款净额候选分开。交付仍为隔离候选；真实模型查询、保存分析引用、完整真实源映射及 WeKnora/Neo4j 图服务待验收，未切换现役服务。

## [0.11.0.0] - 2026-09-20

### Added

- 驾驶舱可汇总历史对话产物，按来源搜索和分页，并将历史文件保存为可编辑副本。
- 支持主动添加 HTML、Word、Excel、CSV 和 PDF；Office/PDF 通过本机 ONLYOFFICE 编辑，保留原件与不可变版本。
- HTML 和文档增加“用 AI 改”入口，复用 DSH 原生会话处理版本副本，收取固定候选、预览后显式保存；带业务绑定的 HTML 仅允许 CSS 修改。

### Fixed

- 修复真实宿主文件服务注入和 AI 会话导航；历史读取失败或截断时提供明确提示。
- 保存回执丢失时保留后续未保存修改；拒绝错配回执和旧草稿误确认，文档服务保存失败或版本冲突后可恢复操作。
- 页面候选重新核验权限并恢复过期预览；AI 任务支持分页与并发幂等，损坏的 Office 文件在入库与候选收取时拒绝。

真实模型验收因缺少凭据未完成；扫描 PDF 的 OCR、复杂 Office 兼容和 PDF 浮动菜单异常仍开放。产品仍 PARTIAL，本批 PR 不代表现役切换或真实业务验收通过。

## [0.10.0.0] - 2026-09-19

### Added

- 会话交付集中出现在驾驶舱产物列表，HTML 可预览后显式保存为可编辑副本，保留来源会话与文件路径。
- 已保存 HTML 支持可靠文字节点点选、精确替换、预览确认与历史回退；看板支持本页修改标题、正文、展示属性和数据来源。

### Changed

- 驾驶舱采用可折叠产物列表与响应式编辑侧栏，小屏可切换画布和设置；未保存修改与未知保存回执统一提示并保护。
- 看板点选只选择组件，显式「用 AI 改」才进入原生对话；保存继续由用户确认。

### Fixed

- 修复看板属性保存被拒绝、新看板预览被旧 HTML 选择遮挡，以及确认冲突后无法取消和刷新最新版本。
- 修复 HTML 入库时脚本依赖顺序被改变，以及重新进入驾驶舱后原始 HTML 空白。
- 产品仍 PARTIAL；本批交付 PR，不代表现役、真实模型或真实业务验收通过。
...
- 账本：完整 M2 Figma 收口。完整属性／整板编辑／DSH 稿／真数据瀑布保持缺口（用户确认也算关）。未知金额 WATERFALL 仍 422。**不是 Figma 通过**。产品仍 PARTIAL。
...
- DSH 钉升到 **0.1.6-alpha.1**（`0a15e36e`）。隔离 pipeline `--prepare/--check` 通过；隔离 14327 未认证 401。合入不等于切换现役 6677。不上公网。
...
- S4 固定候选 `775934f5`／6677 PID 12386：隔离 G2/G3 与已加载插件 HTTP 主链通过；只读确认说明板 v15／原板 v9。G1 真模型、现场确认保存、完整 G5 仍 NOT_RUN。不升版本、未发布。
- S5 现役驾驶舱入口为 `http://127.0.0.1:6677/`；比赛看板 `http://127.0.0.1:15173/` 只作独立探活，不是 T15 运营路径。4325 历史清单不再作为现役入口。回退与 G6 草稿见 S5 交接文档。U1 不代签。
- 比赛看板入口改为 `http://127.0.0.1:15173/`，与 `start-stack` 默认前端端口一致，避开通用 Vite 5173。旧 CRM 后端仍是 8000；DSH 合成 HTTP 仍是 18082。
- 侧栏底部新增「比赛看板」入口，在新标签页打开比赛看板前端；看板仍是独立应用与独立登录，DSH 不内嵌、不代理。
- 驾驶舱生成前复用组件目录校验属性和布局，明确指出非法属性或重叠组件；配置不自动改写、不自动重试。修正布局可保留购买频次漏斗，失败不授权调用文件或 shell 工具。#153（`698278e`）有界真实模型命中布局重叠预检并保留 FUNNEL；Chrome 取消未保存预览。TABLE `show_values` 现场未复现。
- 驾驶舱窄屏导航改为覆盖展开，保留单列画布宽度，关闭后恢复阅读；只在业务适配层生效，保留 DSH 原生导航与聊天。
- 取消已应用草稿时明确提示已保存内容不能通过取消撤销；连接异常改为中文，丢失保存回执继续保留待核对状态。
- 本地 B0 完整检查与真实 Chrome 定点复验完成；历史 85 跳转仍未定因，整体 PARTIAL，不代替 S4 或本人 UAT。不升版本、未发布。

## [0.8.0.1] - 2026-09-16

### Added

- 运营可从原生「我的驾驶舱」打开「人群行动」，不必再绕旧 overlay。自动发送仍关闭。未确认的看板预览条切页后仍可见。#166（`b78beffa`）。非正式 release。

## [0.8.0.0] - 2026-09-12

### Added

- 驾驶舱按 BoardSpec 生成：聊完后点「生成驾驶舱」，确认才写入。点选卡片改标题/口径，问数只刷新绑定数字，回退上一版。数字来自已核验结果的 `result_id`，没有结果就不画 0%。
- 侧栏驾驶舱默认一块合成样例板（标题带「样例」、证据标 `SYNTHETIC`），生成后替换。飞书文档和多维表只写链接占位，不接飞书 token。
- 数据员工是旁路面板，不另开一套聊天。

### Fixed

- 刷新 facts 按 `result_id` 绑定，不再写成固定 `r1`。
- HTML 沙箱去掉脚本；srcdoc 不能关掉 CSP 包装；刷新不跟随跳转、不打内网。
- 回退走 history 上一版，不是写死 v1。
- 底栏有驾驶舱面板就切过去，没有 layout 才打开 overlay。
- dsh-dev 日常只认固定 `runtime` 目录，不再抄一次性 `runtime-*` 名字。

## [0.7.0.0] - 2026-09-09

- 2026-09-14 现有成果收口（#131–#134）：VERSION仍为0.8.0.0，不是正式release或运行环境切换。最新Git与CI看STATUS；以下既有条目保留其历史时点。
- 组件库优先生成／点选编辑：六类基础组件及流程、时间线、贡献瀑布、购买频次漏斗沿登记属性、品牌令牌和结果绑定生成，缺数不伪造零。原生聊天和画布复用同一DSH Agent；不把静态样例或工具测试称为完整真实模型验收。
- 看板预览、确认、取消、版本回退及自由布局接服务端SQLite；权限／会话／base_version、幂等与冲突保护保持，模型不获确认写权限。保存结果未知显示“待核对”，支持只读检查或显式同键重试，取消草稿不冒充撤销已应用版本。
- 保存／读取有效快照后同步选择器标题和版本；后续GET/LIST失败不退回旧版本、不遗漏已知新板。未知草稿不列为已保存。
- 保存未知、主动刷新与布局编辑后的键盘焦点可恢复；Tab/Enter能继续操作，迟到响应不抢原生聊天已有焦点。完整触控／读屏器与正式壳综合验收仍开放。
- 计算事实增加逐日趋势、可对账渠道贡献和同人群购买频次；保留旧版本读取。Python/JS差额校验采用一致的四位舍入判定，拒绝中点伪造；不改已发布数值或凭空补桶。
- dsh-dev重载就绪绑定本次owned启动，等清理完成再启动、退出快失败、分层取消回收自己的子进程；不误认陌生401、不扫PID或删陌生锁。合成服务显式允许6677页面来源但禁止自身绑定6677；配置测试按明确Python3.14入口／PATH解析，错误配置硬失败。
...
- 2026-09-15 S3 原型与正式壳续验（文档，不升版本）：指定 Figma 点击矩阵通过；正式壳 B3 九类状态代表路径及桌面／390 有实测。说明板终态 v9。窄屏侧栏、拒绝取消文案、完整分支×视口矩阵仍开放。不勾 G1–G6，不代签 UAT。本机账本 `.context/checks/` 不进 Git。
- 2026-09-15 S3 正式壳交叉补验（文档，不升版本）：B3 核心恢复分支补齐桌面／390 交叉，恢复按钮忙态 12 次实点无额外请求。说明板有意保存至 v14；85 桌面取消忙态意外返回对话为 INCONCLUSIVE。随后 88 写入前阻断未生效，实际新增同正文 v15，记 SETUP_FAILED，不算新通过。不勾 G1–G6。本机 `.context/checks/` 不进 Git。
